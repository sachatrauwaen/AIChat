...
{
    class SetModuleTool : ITool
    {
        public string Name => "Set Module content";

        public string Description => "Set content of module";

        public MethodInfo Function => typeof(SetModuleTool).GetMethod(nameof(SetModule));
            ...
            if (!string.IsNullOrEmpty(module.DesktopModule.BusinessControllerClass) && module.DesktopModule.IsPortable)
            {
                try
                {
                    ...
                    if (typeof(IPortable).IsAssignableFrom(businessControllerType))
                    {
                        ...
                        content = $"<htmltext><content><![CDATA[{WebUtility.HtmlEncode(content)}]]></content></htmltext>";

                        portable.ImportModule(module.ModuleID, content, module.DesktopModule.Version, PortalSettings.Current.UserId);
